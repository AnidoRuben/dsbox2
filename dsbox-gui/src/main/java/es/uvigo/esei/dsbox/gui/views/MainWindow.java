...
import es.uvigo.esei.dsbox.gui.views.panels.VBoxDriverSpecEditorPanel;
...
//        configureCommand = CommandPair.create("Configure", "icons/CONFIG.png", (ActionEvent ae) -> this.manageProperties());        
...
//                pasteCommand.getMenuItem());
    ...
    private ToolBar createToolBar() {
        ...
        toolBar.getItems().add(new Separator(Orientation.VERTICAL));
        toolBar.getItems().addAll(vboxDriverCommand.getButton());
        toolBar.getItems().add(new Separator(Orientation.VERTICAL));
...
//                                          -> habilitar pasteCommand cuando se haya copiado (y deshabilitar copyCommand)
    ...
    private void activateSimulationMode() {
        this.inEditionMode = false;
        this.isEdited = false;
        editor.activateSimulationMode();
...
//    }
    ...
    private void configVMDriver() {
        ...
        VirtualBoxDriverSpec spec = controller.getVBoxDriverSpec();

        Stage stage = new Stage();
        stage.setTitle("VirtualBox driver configuration");

        VBoxDriverSpecEditorPanel editPanel = new VBoxDriverSpecEditorPanel(spec);
...
//    }
    ...
    private void about() {
        Alert alert = new Alert(AlertType.INFORMATION, "DSBOX. 2015-2016", ButtonType.OK);
