...
//        });
    ...
    private Node createHostLinksPanel() {
        ...
        destinationColumn.setCellValueFactory((TableColumn.CellDataFeatures<Link, String> cellData) -> {
            ...
            String destinationDescription = network.getName();
    ...
    private List<Link> cloneLinkList(List<Link> links) {
        if (links != null) {
            ...
            for (Link link : links) {
                Link cloned = new Link(link.getHost(), link.getNetwork(), link.getInterfaceOrder(), link.getIpAddress(), link.getNetMask(), link.getBroadcast());
